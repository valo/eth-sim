...
fn run_rpc_simulation(mut tx: Transaction, current_latest_block: Block<H256>) -> SimulationResult {
    ...
    }).join().expect("RPC thread panicked!")
}
            ...
            if let Some(tx) = stream.next().await {
                ...
                match rpc_simulation_result.result {
                    Result::Ok(result) => {
                        ...
                        println!("Elapsed time: {} ms. {:.2} MGas/s", rpc_simulation_result.elapsed_time.as_millis(), result.result.gas_used() as f64 / 1000000.0 / rpc_simulation_result.elapsed_time.as_secs_f64());
                    }
                    Result::Err(e) => {
                ...
                match result {
                    Result::Ok(result) => {
                        ...
                        let speedup = rpc_simulation_result.elapsed_time.as_secs_f64() / reth_elapsed_time.as_secs_f64();
